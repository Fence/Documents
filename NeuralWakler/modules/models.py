...
class NeuralWalker(object):
    ...
    def __init__(self, model_settings):
        ...
        assert(self.drop_out_rate <= numpy.float32(1.0))

        self.rnd_gen = RandomStreams(seed=12345)
        self.drop_out_layer = self.rnd_gen.uniform((self.dim_model,)) < self.drop_out_rate
        self.drop_out_layer_gen = theano.function([], self.drop_out_layer)
        #
        # self.dim_world, world
        print "dim of model, lang and action is : ", self.dim_model, self.dim_lang, self.dim_action
        #
        ...
        self.W_dec = theano.shared(
            utils.sample_weights(
                self.dim_lang+3*self.dim_model, 4*self.dim_model
        ...
        #
        self.save_file = model_settings['save_file']
        if model_settings['load_model']:
            self.load_model()
    ...
    def func_enc_forward(self, xt, htm1, ctm1):
        ...
        post_transform = self.b_enc_forward + theano.dot(
            tensor.concatenate([xt, htm1], axis=0),
            self.W_enc_forward
        )
    ...
    def func_dec(self, xt, htm1, ctm1):
        ...
        post_transform = self.b_dec + theano.dot(
            tensor.concatenate([htm1, zt], axis=0), self.W_dec)
        gate_input = tensor.nnet.sigmoid(
            post_transform[:self.dim_model]
        )
        gate_forget = tensor.nnet.sigmoid(
    ...
    #@profile
    def compute_loss(self, seq_lang, seq_action):#seq_world, seq_action):
        ...
        xt_world = self.xt_world0 # theano.dot(seq_world, self.Emb_dec) # modified by Fence
        #
        ...
        post_transform = theano.dot(
            theano.dot( #xt_world + theano.dot(
                tensor.concatenate(
        ...
        loglikelihood_action = tensor.mean(loglikelihood_path)
        #
        #pdb.set_trace()
    ...
    def load_model(self):
        ...
        if os.path.exists(self.save_file):
            ...
            self.Emb_enc_forward.set_value(model_dict['Emb_enc_forward']) 
            self.W_enc_forward.set_value(model_dict['W_enc_forward']) 
            self.b_enc_forward.set_value(model_dict['b_enc_forward']) 

            self.Emb_enc_backward.set_value(model_dict['Emb_enc_backward']) 
            self.W_enc_backward.set_value(model_dict['W_enc_backward']) 
            self.b_enc_backward.set_value(model_dict['b_enc_backward']) 
            
            self.W_att_scope.set_value(model_dict['W_att_scope']) 
            self.W_att_target.set_value(model_dict['W_att_target']) 
            self.b_att.set_value(model_dict['b_att']) 
            
            self.W_dec.set_value(model_dict['W_dec']) 
            self.b_dec.set_value(model_dict['b_dec']) 
            self.W_out_hz.set_value(model_dict['W_out_hz']) 
            self.W_out.set_value(model_dict['W_out']) 
        else:
            print "\nModel doesn't exist.\n"