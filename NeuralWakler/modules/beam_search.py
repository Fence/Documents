...
class BeamSearchNeuralWalker(object):
    ...
    def set_encoder(self, seq_lang_numpy, seq_action_numpy):#, seq_world_numpy):
        #
        self.seq_lang_numpy = seq_lang_numpy
        self.seq_action_numpy = seq_action_numpy
    ...
    def init_beam(self):#, pos_start, pos_end):
        ...
        self.item  = {
            ...
            #'feat_current_position': numpy.copy(self.seq_world_numpy[0, :]),
            #
        ...
        self.beam_list.append(self.item)
    ...
    def decode_step(self, htm1_action, ctm1_action):# feat_current_position,
        #
        #import pdb
        #pdb.set_trace()
        #xt_action = numpy.dot(feat_current_position, self.model['Emb_dec'])
        xt_action = [] 
        ...
        post_transform_prob = numpy.dot(
            #xt_action + numpy.dot(
            numpy.dot(
                numpy.concatenate((ht_action, zt_action), axis=0), self.model['W_out_hz']),
            self.model['W_out']
        )
        #
    ...
    def search_func(self):
        ...
        xt_item, ht_item, ct_item, probt_item, log_probt_item = self.decode_step(
                self.item['htm1'], self.item['ctm1'])
        # find the first action and then the rest
        #if len(self.seq_lang_numpy) < 50:
        word_prob = probt_item[:len(self.seq_lang_numpy)]
        #else:
        #    word_prob = probt_item
        idx = numpy.argmax(word_prob)
        # see if exist an action
        if word_prob[idx] > threshold:
            action_list.append(self.seq_lang_numpy[idx])
            # find other actions
            for i,wp in enumerate(word_prob):
                if i != idx and word_prob[i] > word_prob[idx]*other_action_rate:
                    action_list.append(self.seq_lang_numpy[i])
        """

        # decode actions one by one
        
        while counter < max_count:
            #print 'counter',counter
            xt_item, ht_item, ct_item, probt_item, log_probt_item = self.decode_step(
                self.item['htm1'], self.item['ctm1'])
            self.item['htm1'] = ht_item
            self.item['ctm1'] = ct_item
            word_prob = probt_item[begin_idx:len(self.seq_lang_numpy)]
            
            idx = numpy.argmax(word_prob)
            # see if exist an action
            if word_prob[idx] > threshold:
                action_list.append(self.seq_lang_numpy[idx+begin_idx])
            begin_idx = begin_idx + idx + 1
            counter += 1
            if begin_idx >= len(self.seq_lang_numpy) - 1:
                break
